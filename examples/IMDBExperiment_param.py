...
# test s
nsb_params = [4, 8, 16]
nsb_results_df = pd.DataFrame(columns=['rep', 'epoch', 'num_examples', 'accuracy',
...
for state_bits in nsb_params:
    ...
    for i in range(50):
        ...
        nsb_results_df = nsb_results_df.append(row, ignore_index=True)
        nsb_results_df.to_csv("imdb_nstatebits_5000examples_"+str(exp_id)+".csv", index = False)